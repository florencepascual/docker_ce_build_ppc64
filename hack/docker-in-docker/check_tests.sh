...
then
    echo "There is no file ${PATH_TEST_ERRORS} with the errors." 2>&1 | tee -a ${PATH_LOGS}/${NAME_LOG_PROWJOB}
    # push ERROR
    CHECK_TESTS_BOOL="ERR"
fi

ls ${DIR_TEST} 2>&1 | tee -a ${PATH_LOGS}/${NAME_LOG_PROWJOB}
...
echo ${NB_BUILD_LOGS} 2>&1 | tee -a ${PATH_LOGS}/${NAME_LOG_PROWJOB}
echo ${NB_TEST_LOGS} 2>&1 | tee -a ${PATH_LOGS}/${NAME_LOG_PROWJOB}
...
then
    ...
    then
        ...
        echo "# Check the file #" 2>&1 | tee -a ${PATH_LOGS}/${NAME_LOG_PROWJOB}
        TOTAL_ERRORS=$(eval "grep -c 1 ${PATH_TEST_ERRORS}")
        if [[ ${TOTAL_ERRORS} -eq 0 ]]
        then
            echo "There is no error in the test log files. We can push to the shared COS Bucket. " 2>&1 | tee -a ${PATH_LOGS}/${NAME_LOG_PROWJOB}
            # push NO ERROR
            CHECK_TESTS_BOOL="NOERR"
        else 
            echo "We have every log but there are errors in the test log files. " 2>&1 | tee -a ${PATH_LOGS}/${NAME_LOG_PROWJOB}
            # push ERROR
            CHECK_TESTS_BOOL="ERR"
        fi
    else 
        # error and push to cos bucket ERR
        echo "There are build or test log files missing. " 2>&1 | tee -a ${PATH_LOGS}/${NAME_LOG_PROWJOB}
        # check which test log files are missing
        for DISTRO in ${DISTROS}
        do
            TEST_LOG="${DIR_TEST}/test_${DISTRO}.log"
            find ${TEST_LOG} 2>&1 | tee -a ${PATH_LOGS}/${NAME_LOG_PROWJOB}
            if [[ $? -ne 0 ]]
            then
        ...
        TOTAL_ERRORS=$(eval "grep -c 1 ${PATH_TEST_ERRORS}")
        echo "There are ${TOTAL_MISSING} test log files missing and there are ${TOTAL_ERRORS} errors for the existing test log files." 2>&1 | tee -a ${PATH_LOGS}/${NAME_LOG_PROWJOB}
        # push ERROR
        CHECK_TESTS_BOOL="ERR"

    fi
    echo "There are no build logs or no test logs." 2>&1 | tee -a ${PATH_LOGS}/${NAME_LOG_PROWJOB}
    # push ERROR
    CHECK_TESTS_BOOL="ERR"
fi
