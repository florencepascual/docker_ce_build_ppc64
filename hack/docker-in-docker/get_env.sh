...
s3fs --version 2>&1 | tee -a ${PATH_LOGS}/${NAME_LOG_PROWJOB}
...
ls ${PATH_COS}/s3_${COS_BUCKET}/prow-docker/ 2>&1 | tee -a ${PATH_LOGS}/${NAME_LOG_PROWJOB}
...
then
    ...
    then
        echo "The containerd packages have been copied." 2>&1 | tee -a ${PATH_LOGS}/${NAME_LOG_PROWJOB}
        exit 0
    else
        echo "The containerd packages have not been copied." 2>&1 | tee -a ${PATH_LOGS}/${NAME_LOG_PROWJOB}
        exit 1
    fi
else
    echo "The env.list and the dockertest directory have been copied." 2>&1 | tee -a ${PATH_LOGS}/${NAME_LOG_PROWJOB}
    exit 0
fi