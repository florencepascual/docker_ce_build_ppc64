  ...
  # docker login
    ...
    do
      ...
      do
        ...
        docker run --env SECRET_AUTH --env DISTRO_NAME -d -v /workspace/docker-ce-${DOCKER_VERS}:/workspace/docker-ce-${DOCKER_VERS} -v /workspace/containerd-${CONTAINERD_VERS}:/workspace/containerd-${CONTAINERD_VERS} -v /workspace/test/src/github.ibm.com/powercloud/dockertest:/workspace/test/src/github.ibm.com/powercloud/dockertest -v /workspace/docker_ce_build_ppc64:/workspace/docker_ce_build_ppc64 --privileged --name ${CONT_NAME} ${IMAGE_NAME} --entrypoint ./${PATH_SCRIPTS}/test_launch.sh &> ${DIR_TEST}/${TEST_LOG}
