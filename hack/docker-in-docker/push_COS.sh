...
then
    ...
    echo "${PATH_COS}/s3_${COS_BUCKET_PRIVATE}/prow-docker/prow-job-* deleted"    
...
echo "-- ERR and NOERR ppc64le-docker --" 
...
ls ${PATH_COS}/s3_${COS_BUCKET_PRIVATE}/prow-docker/TEST 

if [[ test -d ${PATH_COS}/s3_${COS_BUCKET_PRIVATE}/prow-docker/TEST ]]
then
    echo "Packages in the private COS Bucket" 
    BOOL_PRIVATE=0
else
    echo "Packages not in the private COS Bucket" 
    exit 1
fi
...
then
    ...
    then
        ls ${PATH_COS}/s3_${COS_BUCKET_SHARED} 
        echo "No error in the tests and shared bucket mounted." 
        exit 0
    else 
        echo "No error in the tests but shared bucket not mounted." 
        exit 1
    fi
elif [[ $1 -eq "ERR" ]] && [[ BOOL_PRIVATE -eq 0 ]]
    echo "There was some errors in the test, the packages have been pushed only to the private COS Bucket." 
    exit 0
fi
